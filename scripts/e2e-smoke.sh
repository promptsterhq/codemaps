...
echo "== lenses (CLI) against $TARGET =="
cd "$TARGET"
...
check "contracts"   node "$CLI" contracts
...
echo "== MCP server over stdio (all seven tools) =="
MCP_OUT=$( (printf '%s\n' \
...
if [ "$MCP_OUT" = "contracts,guardrails,impact,locate,orient,risk,security|true" ]; then
  echo "  ✅ MCP: seven tools listed + live risk call"; PASS=$((PASS+1))
else
